...
The clearest evidence for that is that their place classifications are now the *same set*: `ControlSub` is `BasicBlock` / `FunctionStart` / `FunctionEnd`, which is `PlaceType` minus `Resources` — and `Resources` is the other arm of the CVN's `PlaceKind`. `ControlSub` names *roles*, not granularities, and each variant is a distinction some analysis makes:
